...
constexpr size_t GLOBAL_REORDER_CAP = 8192;
...
MPMCQueue<OrderedOutputItem, GLOBAL_REORDER_CAP> g_reorder_queue;
std::atomic<size_t> g_reorder_queue_depth{0};
std::thread g_reorder_thread;
std::atomic<bool> g_reorder_stop{false};
Stage* g_target_stage = nullptr;   // единственная терминальная стадия
...
bool pop_free_slot(int& slot) {
    ...
    // Восстанавливаем матрицу до исходного состояния
    buffer[slot].mat.create(g_cfg.frame_h, g_cfg.frame_w,
...
// ================= GLOBAL REORDER WORKER =================
void global_reorder_worker() {
    ...
    auto emit = [&](uint64_t seq, int slot, uint64_t created_ns) {
        // Отправляем кадр в целевую терминальную стадию через обычную очередь
        push_stage_slot(*g_target_stage, slot);
        completed_frames.fetch_add(1, std::memory_order_relaxed);
        g_target_stage->processed_total.fetch_add(1, std::memory_order_relaxed);
        // e2e latency уже добавлена ранее, можно добавить здесь, но уже добавлена в emit исходного кода?
        // В данном месте мы просто передаём кадр дальше.
    };
    ...
    while (!g_reorder_stop.load(std::memory_order_relaxed)) {
        OrderedOutputItem item;
        if (!g_reorder_queue.pop(item)) {
            std::this_thread::sleep_for(std::chrono::microseconds(50));
            continue;
        }
        g_reorder_queue_depth.fetch_sub(1, std::memory_order_relaxed);
...
void forward_after_stage(Stage& stage, int slot) {
    // Если стадия не имеет выхода – такого быть не должно, но на всякий случай освободим слот
    ...
    if (stage.next_stages.size() == 1) {
        ...
        // Если следующая стадия – терминальная, отправляем в глобальный реордерер
        if (next->next_stages.empty()) {
            ...
            item.slot = slot;
            while (!g_reorder_queue.push(item)) std::this_thread::yield();
            g_reorder_queue_depth.fetch_add(1, std::memory_order_relaxed);
            // e2e latency сохраняем здесь
            const uint64_t now_ns = steady_now_ns();
            const uint64_t e2e_ns = (buffer[slot].meta.created_ns > 0 && now_ns >= buffer[slot].meta.created_ns) ? (now_ns - buffer[slot].meta.created_ns) : 0;
            g_e2e_latency_ns.add(e2e_ns);
            // Слот НЕ освобождается – освободит реордерер
        } else {
    ...
    for (size_t i = 0; i < branch_count; ++i) {
        ...
        if (next->next_stages.empty()) {
            ...
            g_reorder_queue_depth.fetch_add(1, std::memory_order_relaxed);
            const uint64_t now_ns = steady_now_ns();
            const uint64_t e2e_ns = (buffer[out_slots[i]].meta.created_ns > 0 && now_ns >= buffer[out_slots[i]].meta.created_ns) ? (now_ns - buffer[out_slots[i]].meta.created_ns) : 0;
            g_e2e_latency_ns.add(e2e_ns);
            // слот не освобождается
        } else {
...
// ================= METRICS EXPORT =================
// (оставляем без изменений, но убираем из вывода reorder_queue_depth, если использовался per-stage)
    ...
    while (true) {
        ...
        if (g_quiet) {
            ...
            std::cout << "[STATS] Input FPS: " << std::fixed << std::setprecision(2) << input_fps
                      ...
                      << " | ReorderQ: " << g_reorder_queue_depth.load(std::memory_order_relaxed)
                      << " | Workers:";
            for (const Stage* s : g_pipeline_stages) {
        ...
        file << ts << "," << std::fixed << std::setprecision(2)
             ...
             << g_reorder_queue_depth.load(std::memory_order_relaxed)
             << "," << std::setprecision(3)
...
int main(int argc, char** argv) {
    try {
        ...
        for (int i = 2; i < argc; ++i) {
            ...
            if (arg == "--quiet" || arg == "-q") g_quiet = true;
        }
        ...
        for (auto& f : buffer) {
            f.meta = {};
            f.mat.create(g_cfg.frame_h, g_cfg.frame_w, CV_MAKETYPE(CV_8U, g_cfg.frame_channels));
        }
        ...
        // Найти терминальную стадию (с пустым next_stages)
        ...
        if (!g_target_stage) {
            std::cerr << "[FATAL] No terminal stage found in pipeline\n";
            return 1;
        }
        ...
        // Запускаем обычные воркеры для всех стадий (включая терминальную)
        for (Stage* s : g_pipeline_stages) {
            start_initial_workers(*s);
        }

        // Запускаем глобальный реордерер
        g_reorder_thread = std::thread(global_reorder_worker);

        // Автоскейлер и метрики
        ...
        while (true) {
            ...
            if (h != static_cast<uint32_t>(g_cfg.frame_h) ||
                ...
                std::cerr << "[WARN] Frame dims mismatch: got " << h << "x" << w << "x" << c
                          << ", expected " << g_cfg.frame_h << "x" << g_cfg.frame_w << "x" << g_cfg.frame_channels << "\n";
                continue;
            }
            ...
            int slot = -1;
            if (!pop_free_slot(slot)) {
                fail_fast_drop("input/free_slots_empty", 0, g_global_seq_counter.load());
            }
            ...
            if (buffer[slot].mat.data == nullptr || buffer[slot].mat.total() * buffer[slot].mat.elemSize() < expected_bytes) {
                std::cerr << "Invalid mat: data=" << buffer[slot].mat.data << ", total=" << buffer[slot].mat.total() << ", elemSize=" << buffer[slot].mat.elemSize() << ", expected=" << expected_bytes << std::endl;
                fail_fast_drop("input/mat_invalid", buffer[slot].meta.frame_id, g_global_seq_counter.load());
            }
